...
- If the student has already submitted an answer, you KNOW what they answered and the feedback they received — use this to personalise your explanation (e.g. "You chose B, which confuses X with Y…").
...
def _is_mandarin_context(*values: str) -> bool:
    """True when the subject/topic label explicitly signals a Mandarin/华文 task.

    Only keyword-match on explicit Mandarin labels — do NOT trigger on any CJK
    ideograph, because bilingual topic names (e.g. "Photosynthesis / 光合作用")
    appear in all subjects and would wrongly force Mandarin replies.
    """
    for v in values:
        if not v:
            continue
...
def _build_question_context(ctx: dict) -> str:
    ...
    # Post-submission context — only present after the student has submitted
    if ctx.get("student_answer") is not None:
        result = "✓ Correct" if ctx.get("is_correct") else "✗ Incorrect"
        parts.append(f"Student's answer: {ctx['student_answer']}  [{result}]")
    if ctx.get("feedback"):
        parts.append(f"Feedback given to student: {ctx['feedback']}")
    return "\n\n".join(parts)
...
--- STUDY NOTES (supplementary reference): {lesson['title']} ---
