...
def generate_quiz(
    ...
    if lesson_id:
        try:
            cached_res = (
                ...
                .select("id, topic, difficulty_level, question_type, questions_jsonb, language")
            ...
            if cached_res.data:
                ...
                print(f"-> Quiz cache hit for '{topic}' ({question_type}/{difficulty}) — skipping LLM.")
                return {
                    "id": cached["id"],
                    "topic": cached.get("topic", topic),
                    "difficulty": cached.get("difficulty_level", difficulty),
                    "question_type": cached.get("question_type", question_type),
                    "questions": cached.get("questions_jsonb", []),
                }
        except Exception as e:
            print(f"-> Quiz cache lookup failed ({e}); regenerating.")
    ...
    try:
        res = call_llm(prompt, want_json=True, temperature=0.4, max_tokens=6000)
