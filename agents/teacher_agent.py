...
def _tool_query_mastery(args: dict) -> dict:
    ...
    try:
        ...
        res = q.order("mastery_level", desc=False).limit(40).execute()
        rows = [{
            "student": name_by_id.get(r["student_id"], "Unknown"),
...
- get_student_roster {}  -> full list of all students with average mastery %.
- query_mastery {"student"?,"subject"?,"topic"?,"threshold"?}  -> filtered mastery records; threshold=0.5 returns only students below 50%.
